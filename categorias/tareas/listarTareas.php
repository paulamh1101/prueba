		...
		<tbody>
			...
			<?php
				$sql = "SELECT id, start, end, title, color, descripcion, prioridad, estado, creado_por, fecha_creacion, comentario FROM tareas as A
						...
						ON A.id = C.id_tareas
						WHERE tipo = 'tarea' AND C.id_usuarios = '".$_SESSION['usuario']."'
						UNION 
						...
						INNER JOIN tareas_usuarios AS C
						ON B.id = C.id_tareas
						WHERE tipo = 'tarea' AND C.id_usuarios = '".$_SESSION['usuario']."' order by fecha_creacion desc";
				...
				foreach($events as $row):
					echo '<tr>';
						echo '<td><a href="detalleTarea.php?id='.$row["id"].'&pag=tarea">'.$row["id"].'</a></td>';
