...
if(isset($_GET['leido'])) {
  ...
  $usuariod = mysqli_real_escape_string($connect, $_GET['usuario']);
      ...
      <h1>
        ...
        <small>
			<?php
				...
				$chat = mysqli_query($connect, "SELECT count(leido) as leido FROM chats WHERE leido = 0 and para = $usuario");
				$cha = mysqli_fetch_array($chat);
				if($cha['leido'] != ""){ echo $cha['leido']; }else{ echo "0"; }
			?>
				nuevos mensajes
		</small>
      </h1>
            ...
            <div class="box-footer">
                ...
                <div class="input-group">
                  ...
                  <input type="text" name="mensaje" id="mensaje" placeholder="Escribe un mensaje" class="form-control">
                      <span class="input-group-btn">
                        <input type="button" value="Enviar" name="btn_enviar" id="btn_enviar" class="btn btn-primary btn-flat"></button>
                      </span>
                </div>
              </form>
              ...
              <?php
				/*if(isset($_POST['enviar'])) {
					...
					$de = $_SESSION['id'];
					$para = mysqli_real_escape_string($connect, $_GET['usuario']);
...
<script type="text/javascript">
	$(document).on('click', '#btn_enviar', function() {
		usuario = document.getElementById('para').value;
		nombre = document.getElementById('nombre_msg').value;
		...
		if(mensaje != ""){
			$.ajax({
				...
				cache: false,
				type: "POST",
				data: {mensaje:mensaje,btn_enviar:btn_enviar,usuario:usuario},
				success: function(html){		
					$("#conversacion").html(html); //Insert chat log into the #chatbox div
					mensaje = document.getElementById('mensaje').value = "";
					//se refresque la pagina cada segundo
					//setInterval(function(){datos(usuario,nombre);}, 1000);
				},
			});
		}
		...
		var req = new XMLHttpRequest();
				...
				req.onreadystatechange = function(){
					if (req.readyState == 4 && req.status == 200){
						document.getElementById('conversacion').innerHTML = req.responseText;			
					}
				}
				
				req.open('GET', 'conversaciones.php', true);
				req.send();
    }*/
