...
//obtener las tareas asigndas de los usuarios
$sql = "SELECT id, title, start, end, color, creado_por, fecha_creacion FROM tareas as A
        INNER JOIN tareas_usuarios AS C 
        ON A.id = C.id_tareas
		WHERE tipo = 'reunion' AND C.id_usuarios = '$id_usuario'";
...
<body>
    ...
    <div class="container">
		...
		<div class="modal fade" id="ModalAdd" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		  <div class="modal-dialog" role="document">
			...
			<form class="form-horizontal" method="POST" action="addReunion.php">
			
			  <div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title" id="myModalLabel">Agregar Reunión</h4>
			  </div>
			  ...
			  <div class="modal-footer">
				...
				<button type="button" id='btn_send' class="btn btn-primary">Guardar</button>
			  </div>
			</form>
			</div>
		  </div>
		  ...
		  <div class="modal-dialog" role="document">
			<div class="modal-content">
				<form class="form-horizontal" id="formulario" method="POST" action="editarReunion.php">
				  <div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title" id="myModalLabel">Modificar Reunión</h4>
				  </div>
				  <div class="modal-body">
				  ...
				  <div class="modal-footer">
					...
					<button type="button" id="btn_modificar" class="btn btn-primary">Guardar</button>
				  </div>
				</form>
				...
				<div class="modal fade" id="ModalUser" tabindex="-2" role="dialog" aria-labelledby="myModalLabel">
				  <div class="modal-dialog" role="document">
					...
					<form class="form-horizontal" id="formulario" method="POST" action="../controller/editarUsuarios.php">
					  ...
					  <input type="hidden" name="id_tarea" class="form-control" id="id_tarea" placeholder="Titulo" readonly>
					  <input type="hidden" name="pagina" class="form-control" id="pagina" value="reuniones">
					  <div class="modal-body">
						  <div class="form-group">
			...
			eventRender: function(event, element) {
				element.bind('dblclick', function() {
					...
					if(creado_por != usuario_sesion){
						$('#id_tarea').prop( "disabled", true );
						$('#titlem').prop( "disabled", true );
						$('#colorm').prop( "disabled", true );
						$('#fecha_creacion').prop( "disabled", true );
						$('#creado_por').prop( "disabled", true );
						$('#startm').prop( "disabled", true );
	...
	$(document).on('click', '#btn_send', function() {
		...
		var id_persona = new Array();
		$(document).ready(function() {
			$("input[id=check_list]:checked").each(function() {
				id_persona.push($(this).val());
			});
		});
		if(title == ""){
			swal({   
			...
			swal({   
				title: "La fecha inicial debe ser mayor o igual a la fecha actual del sistema!",
				type: "warning",   
				confirmButtonColor: "#5cb85c",
				confirmButtonText: "Aceptar",  
				closeOnConfirm: false,
			...
			swal({   
				title: "La fecha final debe ser mayor o igual a la fecha actual del sistema!",
				type: "warning",   
				confirmButtonColor: "#5cb85c",
				confirmButtonText: "Aceptar",  
				closeOnConfirm: false,
			...
			$.ajax({
				url: 'addReunion.php',
				type: "POST",
				data: {color:color, title:title, id_persona:id_persona, start:start, end:end},
				success: function(rep) {
					if(rep == 'OK'){
						location.href="index.php";
					}else{
